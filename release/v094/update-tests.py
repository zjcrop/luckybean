...
test('0.9.4 interface, vector assets and cache manifest', async () => {
  ...
  for (const marker of ['splashSources','applyTheme','restructureBrew','openSegmentedSensory','bindRadar']) assert.ok(extension.includes(marker), marker);
...
test('0.9.4 segmented sensory workflow and draggable radar editors are present', async () => {
  ...
  for (const marker of ['干香','高温','中温','低温','整体强度','跳过当前温区','拖拽圆点标定各轴强度','自然语言札记']) assert.ok(extension.includes(marker), marker);
