...
app = read('src/app.js')
if 'lb-brew-five-row' in app and 'versionCode 102311' in read('android/app/build.gradle'):
    print('Small-brew five-row refactor already applied; nothing to do.')
    raise SystemExit(0)
...
app = replace_once(app, "waterProfileId: 'auto', waterVolumeL: 5,", "waterProfileId: 'plain', waterVolumeL: 5,", 'default water')
app = replace_once(
    app,
...
app = replace_once(
  ...
  if (!preserveOverlay) closeOverlay();
  state.page = page;""",
    'switchPage state boundary'
)

helpers = r'''function brewProfileSelection() {
...
function renderBrew() {'''
app = replace_once(app, 'function renderBrew() {', helpers, 'brew helpers')
...
new_template = r'''  const customWaterLabel = currentWater === 'custom' ? `${settings.customWater?.name || '自定义'} · TDS ${Number(settings.customWater?.tds || 85)}` : '';
...
app = regex_once(
    app,
    r"  const customWaterLabel = currentWater === 'custom'.*?\n  container\.innerHTML = `.*?`;\n(?=  \$\('#brewBean'\))",
    new_template,
...
app = replace_once(
    ...
    'bean change state reset'
)

# Environment fields are no longer inline; remove the old inline listeners.
app = regex_once(
    app,
    r"  \['ambientTemperatureC','relativeHumidityPct','initialBedTemperatureC'\]\.forEach\(id => \$\('#'\+id\)\?\.addEventListener\('change', async \(\) => \{.*?\n  \}\)\);",
    '',
    'inline environment listeners'
)
...
app = replace_once(
  ...
  $('#brewFilterPaper')?.addEventListener('change', async event => {
    ...
    'transient dripper selection'
)

# No material editor belongs in the small-brew page anymore.
app = regex_once(
    app,
    r"  \$\('#brewDripperMaterial'\)\?\.addEventListener\('change', async event => \{.*?\n  \}\);",
    '',
    'small-brew material listener'
)
...
app = replace_once(
    ...
    "  $('#brewWaterProfile')?.addEventListener('change', async event => { state.settings.brew.waterProfileId = event.target.value; await saveSettings(); if (event.target.value === 'custom') openCustomWaterDialog(); else renderBrew(); });",
    'water default persistence anchor'
)

app = regex_once(
    app,
    r"  const bindCoolingMode = \(id, which, modeKey\) => \{.*?\n  bindCoolingMode\('tailCoolingMode', 'tail', 'tailCoolingMode'\);",
...
app = replace_once(
    ...
    'brew input profile/gear/cooling'
)
...
app = replace_once(
    ...
    'do not persist transient profile/dripper'
)
...
if 'id="brewSegments"' in app:
    raise RuntimeError('brewSegments remains in app.js')
if "select.addEventListener('pointerdown', reopenCustom)" in app:
    raise RuntimeError('legacy cooling pointerdown trap remains')
write('src/app.js', app)

# Gear decorator must derive material/angle/bypass from the exact recommended/manual dripper, without a material selector.
gear = read('src/features/gear-matching-controller.js')
gear = replace_once(
    gear,
...
write('src/features/gear-matching-controller.js', gear)

repair = read('src/features/interaction-repair-controller.js')
repair = repair.replace('<strong>豆卡自动推荐</strong>', '<strong>模型推荐结果</strong>')
write('src/features/interaction-repair-controller.js', repair)

css = read('src/ui/interaction-repair.css')
marker = '/* LuckyBean small-brew five-row contract */'
if marker not in css:
    css += r'''

/* LuckyBean small-brew five-row contract */
.lb-brew-five-row{display:grid;gap:10px}
.brew-row-primary{align-items:end}
.brew-primary-field>span,.brew-profile-field>span{font-size:.78rem;letter-spacing:.04em;opacity:.72}
.brew-large-control,.brew-large-profile{min-height:58px!important;font-size:clamp(1.32rem,5vw,1.9rem)!important;font-weight:720!important;line-height:1.1!important;padding:.55rem .65rem!important}
.brew-row-secondary{grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:7px!important;align-items:end}
.brew-row-secondary .field>span{font-size:.72rem;opacity:.7}
.brew-small-control{min-width:0!important;font-size:.76rem!important;padding-inline:.35rem!important;white-space:nowrap;text-overflow:ellipsis}
.brew-action-strip{gap:7px!important}
.brew-menu-button{min-width:0!important;min-height:42px!important;font-size:.78rem!important;padding:.48rem .45rem!important;white-space:normal!important;text-align:center!important}
.brew-profile-row{grid-template-columns:minmax(0,1fr)!important}
.brew-profile-field{width:100%}
.lb-choice-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin:14px 0 4px}
.lb-brew-five-row [data-brew-row="profile"]{margin-top:2px;padding-top:6px;border-top:1px solid var(--line-color,rgba(128,128,128,.22))}
@media (max-width:520px){
  .brew-row-secondary{gap:5px!important}
  .brew-small-control{font-size:.7rem!important;padding-inline:.22rem!important}
  .brew-menu-button{font-size:.72rem!important;padding-inline:.28rem!important}
  .brew-large-control,.brew-large-profile{min-height:54px!important;font-size:1.42rem!important}
}
'''
write('src/ui/interaction-repair.css', css)

# Static regression for the new contract.
...
'''
write('tests/v123e-small-brew-five-row-regression.mjs', test)

runner = read('scripts/run-static-tests.mjs')
if 'v123e-small-brew-five-row-regression.mjs' not in runner:
    runner = replace_once(
        runner,
...
# Internal Android build revision only; public version name remains 1.23E.
for path in ['android/app/build.gradle', '.github/workflows/build-main.yml']:
    text = read(path)
    if '102310' in text:
        text = text.replace('102310', '102311')
    write(path, text)
for path in (ROOT / 'tests').glob('*.mjs'):
    text = path.read_text(encoding='utf-8')
    if '102310' in text:
        path.write_text(text.replace('102310', '102311'), encoding='utf-8')

workflow = read('.github/workflows/build-main.yml')
anchor = "          unzip -p \"$apk\" assets/web-cache/src/services/brew-analysis-service.js | grep -Fq 'dripperSnapshot'\n"
if "grep -Fq 'brewProfileOverride'" not in workflow:
    workflow = replace_once(
        workflow,
        anchor,
        anchor + "          unzip -p \"$apk\" assets/web-cache/src/app.js | grep -Fq 'brewProfileOverride'\n          unzip -p \"$apk\" assets/web-cache/src/app.js | grep -Fq 'data-brew-row=\\\"profile\\\"'\n          if unzip -p \"$apk\" assets/web-cache/src/app.js | grep -Fq 'id=\\\"brewSegments\\\"'; then exit 1; fi\n",
        'APK five-row verification'
    )
write('.github/workflows/build-main.yml', workflow)

print('Small-brew five-row refactor applied.')
