...
export const BREW_OPTIMIZER_VERSION = 'lucky-brew-optimizer-0.9.9.0';
export const TRAJECTORY_MODEL_VERSION = 'lucky-trajectory-inverse-fit-0.9.9.0';
...
const ratioLocked = input.brew?.ratioLocked === true
  ...
  for (let index = 0; index < stages.length; index++) {
    ...
    const duration = index === 0
      ? clamp(bloomBase * controls.bloomFactor * controls.timeScale, 20, 80)
      : clamp((pourSeconds + wait) * controls.timeScale, Math.max(7, pourSeconds), 60);
    cumulative += water;
    stage.index = index + 1;
...
export function optimizeBrewPlan(input = {}, basePlan = {}, options = {}) {
  ...
  plan.explanation = [
    ...
    `逆向求解把正向风味窗口视为必须穿越的目标区，把苦涩、木质与收敛窗口视为避让区；联合搜索研磨、粉水比、温度、流量、等待、总时间和分段水量，使白色萃取轨迹尽量穿过正向区域并避开负面区域。拟合分 ${trajectory.objectiveScore}。`,
