...
export function parseCoffeeOrderText(input=''){
  ...
  const weight=firstMatch(text,[/(?:净含量|规格|重量)[：:\s]*([0-9]+(?:\.[0-9]+)?)\s*(kg|g|克|千克)/i,/([0-9]+(?:\.[0-9]+)?)\s*(kg|g|克|千克)\b/i]);
  ...
  let weightG=null;
  if(weight){const wm=weight.match(/([0-9]+(?:\.[0-9]+)?)\s*(kg|g|克|千克)/i);if(wm){weightG=number(wm[1]);if(/kg|千克/i.test(wm[2]))weightG*=1000;}}
