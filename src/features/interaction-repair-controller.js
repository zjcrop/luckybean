...
function classifyFlavor(name = '') {
  const value = String(name).normalize('NFKC');
  if (/花|茉莉|玫瑰|紫罗兰|洋甘菊|橙花|桂花|薰衣草/.test(value)) return '花香';
  if (/果|莓|柑|橘|柠檬|青柠|桃|苹果|梨|葡萄|芒果|菠萝|百香|李子|樱桃|杏|瓜/.test(value)) return '果香';
  if (/茶|乌龙|红茶|绿茶|伯爵|茶汤/.test(value)) return '茶感';
  if (/香料|肉桂|丁香|胡椒|姜|茴香|豆蔻|香草/.test(value)) return '香料';
  return '其他';
}
...
function repairFlavorPicker(overlay) {
  if (!overlay || overlay.dataset.lbFlavorTaxonomy === '1') return;
  ...
  buttons.forEach(button => buckets.get(classifyFlavor(button.textContent)).push(button));
...
function guideHtml() {
  return `<div class="lb-guide-dialog" role="dialog" aria-modal="true" aria-labelledby="lbGuideTitle">
    ...
    <div class="lb-guide-scroll" tabindex="0">
      ...
      <section><h3>二、小酌</h3><p>先选择豆卡，再设定粉量与粉水比、滤杯与材质、滤纸、冲煮方式、调水方案、微调和目标风味，以及首段/尾段温度。选择“模型推荐”时，系统会把豆卡的烘焙、处理法、豆种和风味特征，与器具修正及目标风味一起交给 BrewProfiles 选择更合适的方案。</p><p>计算结果包含分段注水、温度、时间、方案倾向和三维冲煮轨迹。开始冲煮后可使用计时、分段提示和语音提示；完成后可记录实际豆量与滤纸消耗，并进入品鉴。</p></section>
      ...
      <section><h3>四、器设</h3><p><strong>账户：</strong>建议先注册或登录，以便自动同步豆卡、冲煮、品鉴和设置数据，并查看同步状态及恢复选项。</p><p><strong>私器：</strong>登记常用滤杯、滤杯材质、滤纸和磨豆机。滤杯角度、旁通和滤纸流速等参数会作为冲煮方案匹配的器具修正条件。</p><p><strong>数藏：</strong>可查看风味喜好数字测写、咖啡世界、健康提醒、数据源状态，并进行备份导出、导入和本地数据管理。</p><p><strong>本物：</strong>查看版本、数据结构、数据源与开发信息，并可再次打开本使用说明。网页端与 APP 使用同一套数据结构和主要交互。</p></section>
...
function repairDom() {
  repairFlavorPicker(document.querySelector('[data-overlay="flavors"]'));
  repairAboutSection();
}
