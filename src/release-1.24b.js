...
// Centralizes new lifecycle, OCR batch, storage/freshness and brew UI helpers.

export const LB_VERSION = '1.24B';

export const BeanOwnershipStatus = Object.freeze({
  ORDERED: 'ordered',
  OWNED: 'owned',
  ARCHIVED: 'archived'
});

export const LogisticsStatus = Object.freeze({
...
export const RecognitionDocumentType = Object.freeze({
...
export function createRecognitionBatch(images = []) {
  ...
  return {
    ...
    tasks: images.map((image, index) => ({
      ...
      order: index + 1,
      uri: image.uri || null,
      documentType: image.documentType || RecognitionDocumentType.UNKNOWN,
      status: 'pending',
      ocrStatus: 'pending',
      parseStatus: 'pending',
      mergeStatus: 'pending',
...
export function normalizeBeanRecord(bean = {}) {
  return {
    ...
    purchase: {
      currency: bean.purchase?.currency || 'CNY',
      listPrice: bean.purchase?.listPrice ?? null,
      paidPrice: bean.purchase?.paidPrice ?? null,
      quantity: bean.purchase?.quantity ?? 1,
      weight: bean.purchase?.weight ?? null,
      shippingFee: bean.purchase?.shippingFee ?? null,
      discount: bean.purchase?.discount ?? null,
      orderDate: bean.purchase?.orderDate || null,
      merchant: bean.purchase?.merchant || null,
      orderIdHash: bean.purchase?.orderIdHash || null,
      ...bean.purchase
    },
    origin: {
      country: bean.origin?.country || bean.country || null,
      region: bean.origin?.region || bean.region || null,
      subRegion: bean.origin?.subRegion || null,
      farm: bean.origin?.farm || bean.farm || bean.estate || null,
      producer: bean.origin?.producer || bean.producer || null,
      washingStation: bean.origin?.washingStation || null,
      lot: bean.origin?.lot || bean.lot || null,
      ...bean.origin
    },
    varieties: Array.isArray(bean.varieties)
      ? bean.varieties
      : bean.variety
        ? String(bean.variety).split(/[、,，/]/).map((name) => ({ name: name.trim(), ratio: null })).filter((v) => v.name)
        : [],
    processing: {
      process: bean.processing?.process || bean.process || null,
      detail: bean.processing?.detail || bean.processDetail || null,
      ...bean.processing
    },
    storage: {
      currentMode: bean.storage?.currentMode || bean.storageMode || StorageMode.ROOM,
      history: Array.isArray(bean.storage?.history) ? bean.storage.history : [],
...
export function transitionStorage(bean, nextMode, at = new Date()) {
  ...
  const history = [...normalized.storage.history];
  const open = history.length ? history[history.length - 1] : null;
...
export function computeEffectiveAgeDays(bean, now = new Date(), factors = DEFAULT_AGING_FACTORS) {
  ...
  const periods = Array.isArray(b.storage.history) && b.storage.history.length
    ? b.storage.history
  ...
  for (const period of periods) {
    ...
    const factor = Number(factors[period.mode] ?? 1);
    totalMs += (boundedEnd - boundedStart) * factor;
  }
...
export function markBeanInTransit(bean, purchase = {}) {
  ...
  return {
    ...
    logistics: {
      ...
      orderedAt: purchase.orderDate || b.logistics.orderedAt || new Date().toISOString()
    },
    purchase: { ...b.purchase, ...purchase }
  };
}
...
export function beanCardVisualState(bean) {
  const b = normalizeBeanRecord(bean);
  if (b.ownershipStatus === BeanOwnershipStatus.ORDERED || ['ordered', 'shipped', 'in_transit'].includes(b.logistics.status)) {
    return { tone: 'muted', label: '在途', usable: false };
  }
...
export const LOCAL_BREW_METHODS = Object.freeze([
  { id: 'espresso', name: '意式浓缩', category: 'pressure' },
  { id: 'ristretto', name: 'Ristretto', category: 'pressure' },
  { id: 'lungo', name: 'Lungo', category: 'pressure' },
  { id: 'aeropress', name: 'AeroPress', category: 'pressure' },
  { id: 'moka', name: '摩卡壶', category: 'pressure' },
  { id: 'french_press', name: '法压壶', category: 'immersion' },
  { id: 'cold_brew', name: '冷萃', category: 'immersion' },
  { id: 'cold_drip', name: '冰滴', category: 'immersion' },
  { id: 'siphon', name: '虹吸壶', category: 'vacuum' },
  { id: 'cezve', name: '土耳其咖啡', category: 'boiled' },
  { id: 'phin', name: '越南滴滤', category: 'regional' },
  { id: 'south_indian_filter', name: '南印度滤杯', category: 'regional' }
]);
