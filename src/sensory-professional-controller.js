...
const DEFECT_GROUPS = Object.freeze({
  major: { label: '明缺陷', note: '霉腐，坏发酵', tags: ['霉腐', '坏发酵'], multiplier: 2 },
  minor: { label: '暗缺陷', note: '轻微涩', tags: ['轻微涩'], multiplier: 1 }
});
...
const SCORE_STEP = STEPS.length + 1;
const NOTE_STEP = STEPS.length + 2;
const SUMMARY_STEP = STEPS.length + 3;
...
async function selectedBeanId() { return $('#sensoryBeanSelect')?.value || ''; }

async function startMode(mode, existingRecord = null) {
  if (transferBusy) return;
  const beanId = String(existingRecord?.beanId || await selectedBeanId());
  if (!beanId) return;
  ...
  const context = await beanContext(existingRecord?.beanId || beanId);
  const professional = existingRecord?.professionalData || {};
  wizard = {
    editRecordId: String(existingRecord?.id || ''), beanId: String(existingRecord?.beanId || beanId),
    brewSessionId: String(existingRecord?.brewSessionId || brewSessionId), planReference: String(existingRecord?.planReference || planReference),
    profileId: String(existingRecord?.profileId || profileId), source: String(existingRecord?.sensorySource || source),
    bean: context.bean, original: context.original, step: 0,
    selections: Object.fromEntries(STEPS.map(step => [step.id, [...(professional.selections?.[step.id] || [])]])),
    intensities: Object.fromEntries(STEPS.map(step => [step.id, Number(professional.intensities?.[step.id] ?? 7.5)])),
    radar: {
      aroma: Array.isArray(professional.radar?.aroma) ? [...professional.radar.aroma] : [5, 5, 5, 5, 5],
      style: Array.isArray(professional.radar?.style) ? [...professional.radar.style] : [5, 5, 5, 5, 5, 5, 5, 5]
    },
    defects: { major: [...(professional.defects?.major || [])], minor: [...(professional.defects?.minor || [])] },
    selectedRadar: null, naturalNote: String(existingRecord?.naturalNote || ''),
    scoreDelta: clamp(existingRecord?.scoreDelta || professional.scoring?.scoreDelta || 0, -10, 10)
  };
  renderWizard();
}

function closeWizard() { $('#v095ProfessionalOverlay')?.remove(); wizard = null; }

function toggleTag(stepId, tag) {
  const selected = wizard.selections[stepId];
  const index = selected.indexOf(tag);
  if (index >= 0) selected.splice(index, 1); else selected.push(tag);
  renderWizard();
}
...
function radarSvg(group, values, labels, size = 260) {
  const center = size / 2;
  const radius = size * .34;
  const points = values.map((value, index) => { const point = radarPoint(values, index, value, size); return `${point.x},${point.y}`; }).join(' ');
  const grid = [2.5, 5, 7.5, 10].map(level => {
    const ring = values.map((_, index) => { const point = radarPoint(values, index, level, size); return `${point.x},${point.y}`; }).join(' ');
    return `<polygon class="v120-radar-grid-ring" points="${ring}" fill="none"/>`;
  }).join('');
  const axes = values.map((_, index) => {
    const angle = -Math.PI / 2 + index * (Math.PI * 2 / values.length);
    return `<line class="v120-radar-grid-axis" x1="${center}" y1="${center}" x2="${center + Math.cos(angle) * radius}" y2="${center + Math.sin(angle) * radius}"/>`;
  }).join('');
  const text = labels.map((label, index) => {
    const angle = -Math.PI / 2 + index * (Math.PI * 2 / labels.length);
    const r = radius + 22;
    return `<text x="${center + Math.cos(angle) * r}" y="${center + Math.sin(angle) * r}" text-anchor="middle" dominant-baseline="middle" font-size="11">${esc(label)}</text>`;
  }).join('');
  ...
  return `<svg viewBox="0 0 ${size} ${size}" aria-label="感官雷达图" data-v120-radar-chart="${group}">${grid}${axes}<polygon class="v120-radar-value" data-v120-radar-polygon="${group}" points="${points}"/>${nodes}${text}</svg>`;
}
...
function updateRadarChart(root, group) {
  ...
  if (polygon) polygon.setAttribute('points', values.map((value, index) => { const point = radarPoint(values, index, value); return `${point.x},${point.y}`; }).join(' '));
  $$(`[data-v120-radar-node][data-v095-radar-group="${group}"]`, svg).forEach(node => {
    ...
    node.setAttribute('cx', String(point.x)); node.setAttribute('cy', String(point.y)); node.setAttribute('aria-valuenow', Number(values[index]).toFixed(1));
  });
  $$(`.v095-radar-axis[data-v095-radar-group="${group}"]`, root).forEach(button => {
    const index = Number(button.dataset.v095RadarIndex); const output = $('strong', button); if (output) output.textContent = Number(values[index]).toFixed(1);
  });
}

function bindRadarInteractions(root) {
  $$('.v095-radar-axis[data-v095-radar-group]', root).forEach(button => button.addEventListener('click', () => selectRadarAxis(root, button.dataset.v095RadarGroup, Number(button.dataset.v095RadarIndex))));
  $$('[data-v120-radar-node]', root).forEach(node => {
    let activePointer = null;
    const group = node.dataset.v095RadarGroup;
    const index = Number(node.dataset.v095RadarIndex);
    const updateFromPointer = event => {
      const svg = node.closest('svg'); const rect = svg.getBoundingClientRect();
      const x = (event.clientX - rect.left) * 260 / Math.max(1, rect.width); const y = (event.clientY - rect.top) * 260 / Math.max(1, rect.height);
      const geometry = radarPoint(wizard.radar[group], index, 10);
      const projection = (x - geometry.center) * Math.cos(geometry.angle) + (y - geometry.center) * Math.sin(geometry.angle);
      wizard.radar[group][index] = Math.round(clamp(projection / geometry.radius * 10, 0, 10) * 10) / 10;
      updateRadarChart(root, group); selectRadarAxis(root, group, index);
    };
    node.addEventListener('pointerdown', event => { event.preventDefault(); activePointer = event.pointerId; node.setPointerCapture?.(event.pointerId); selectRadarAxis(root, group, index); updateFromPointer(event); });
    node.addEventListener('pointermove', event => { if (activePointer !== event.pointerId) return; event.preventDefault(); updateFromPointer(event); });
    ...
    node.addEventListener('keydown', event => {
      ...
      event.preventDefault(); const delta = ['ArrowUp','ArrowRight'].includes(event.key) ? .1 : -.1;
      wizard.radar[group][index] = clamp(Number(wizard.radar[group][index]) + delta, 0, 10); updateRadarChart(root, group); selectRadarAxis(root, group, index);
    });
  });
}
...
function bindSelectedTagSorting(root, stepId) {
  ...
  const cancelPending = () => {
    ...
    drag.handle?.classList.remove('pressing');
    if (!drag.active) drag = null;
  };
  $$('[data-v120-selected-tag]', list).forEach(chip => {
    chip.addEventListener('click', event => {
      if (event.target.closest('[data-v120-drag-handle]')) return;
      if (chip.dataset.dragJustEnded === '1') { delete chip.dataset.dragJustEnded; return; }
      toggleTag(stepId, chip.dataset.v120SelectedTag);
    });
    const handle = $('[data-v120-drag-handle]', chip);
    if (!handle) return;
    handle.addEventListener('click', event => event.stopPropagation());
    handle.addEventListener('pointerdown', event => {
      if (drag) cancelPending();
      const startX = event.clientX, startY = event.clientY;
      drag = { id: event.pointerId, chip, handle, startX, startY, lastX: startX, lastY: startY, active: false, timer: null };
      handle.classList.add('pressing');
      drag.timer = setTimeout(() => {
        if (!drag || drag.id !== event.pointerId) return;
        drag.active = true;
        chip.classList.add('dragging'); handle.classList.add('active');
    ...
    handle.addEventListener('pointermove', event => {
      ...
      if (!drag.active) {
        if (Math.hypot(event.clientX - drag.startX, event.clientY - drag.startY) > DRAG_CANCEL_DISTANCE) cancelPending();
        return;
      }
      ...
      list.insertBefore(chip, before ? target : target.nextSibling);
    });
    const finish = event => {
      if (!drag || drag.id !== event.pointerId) return;
      clearTimeout(drag.timer);
      const wasActive = drag.active;
      drag.handle.classList.remove('pressing', 'active'); drag.chip.classList.remove('dragging');
      if (wasActive) {
        event.preventDefault(); persist(); drag.chip.dataset.dragJustEnded = '1';
        setTimeout(() => { if (drag?.chip) delete drag.chip.dataset.dragJustEnded; }, 0);
      }
      drag = null;
    };
...
function defectPanel() {
  return Object.entries(DEFECT_GROUPS).map(([key, group]) => `<section class="v095-defect-group"><header><strong>${group.label}</strong><small>${group.note}</small></header>${group.tags.map(tag => `<button type="button" data-v095-defect-group="${key}" data-v095-defect-tag="${esc(tag)}" class="cupping-defect-tag${wizard.defects[key].includes(tag) ? ' active' : ''}" aria-pressed="${wizard.defects[key].includes(tag)}">${esc(tag)}</button>`).join('')}</section>`).join('');
}

function selectedTagHtml(stepId, tag) {
  return `<button type="button" class="cupping-flavor-tag selected v120-selected-tag" data-v120-selected-tag="${esc(tag)}"><span>${esc(tag)}</span><span class="cupping-drag-handle" data-v120-drag-handle aria-label="长按拖动 ${esc(tag)}" title="长按拖动排序" role="button" tabindex="-1"></span></button>`;
}

function poolTagHtml(stepId, tag) {
  const active = wizard.selections[stepId].includes(tag);
  return `<button type="button" data-v095-tag="${esc(tag)}" class="cupping-flavor-tag${active ? ' selected' : ''}" aria-pressed="${active}">${esc(tag)}</button>`;
}
...
function renderWizard() {
  ...
  overlay.id = 'v095ProfessionalOverlay'; overlay.className = 'overlay full v095-professional-overlay';
  const step = STEPS[wizard.step];
  const sensoryStep = wizard.step < STEPS.length, radarStep = wizard.step === RADAR_STEP, scoreStep = wizard.step === SCORE_STEP, noteStep = wizard.step === NOTE_STEP, summaryStep = wizard.step === SUMMARY_STEP;
  const progressTitle = sensoryStep ? `${wizard.step + 1}/${TOTAL_STEPS} · ${step.title}` : radarStep ? `${RADAR_STEP + 1}/${TOTAL_STEPS} · 雷达与缺陷` : scoreStep ? `${SCORE_STEP + 1}/${TOTAL_STEPS} · 打分总结` : noteStep ? `${NOTE_STEP + 1}/${TOTAL_STEPS} · 札记` : `${SUMMARY_STEP + 1}/${TOTAL_STEPS} · 确认结果`;
  const body = sensoryStep
    ? `<section class="v095-wizard-step"><p>${esc(step.subtitle)}</p><small class="v095-sort-hint">已选标签可按右侧圆点长按排序；直接滑动页面不会触发排序。</small><div class="v120-selected-tag-list" data-v120-selected-list="${step.id}">${wizard.selections[step.id].length ? wizard.selections[step.id].map(tag => selectedTagHtml(step.id, tag)).join('') : '<span class="muted small">尚未选择风味标签。</span>'}</div><div class="v095-tag-grid">${step.tags.map(tag => poolTagHtml(step.id, tag)).join('')}</div>${step.intensity ? `<label class="v095-intensity"><span>强度 <output data-v095-intensity-output="${step.id}">${Number(wizard.intensities[step.id]).toFixed(1)}</output></span><input type="range" min="0" max="10" step="0.1" value="${wizard.intensities[step.id]}" data-v095-intensity="${step.id}"></label>` : ''}</section>`
    : radarStep
      ...
      : scoreStep
        ? `<section class="v095-score-stage"><h3>打分总结</h3><p class="muted small">系统依据雷达、风味标签与缺陷计算自动分；可用主观分差表达本次个人判断。</p><div class="question-group score-comparison delta-only"><div class="score-head-row"><span>自动得分</span><span>主观分差</span></div><div class="score-value-row"><strong data-v095-auto-score>${affectiveMappedScore().toFixed(1)}</strong><div class="subjective-delta-control"><strong data-v095-score-delta>${wizard.scoreDelta >= 0 ? '+' : ''}${Number(wizard.scoreDelta).toFixed(1)}</strong><input type="range" min="-10" max="10" step="0.5" value="${Number(wizard.scoreDelta)}" data-v095-score-delta-input aria-label="设置杯测主观分差"></div></div><div class="score-derived-row"><small>折算总分</small><small data-v095-subjective-score>${subjectiveScore().toFixed(1)}</small></div></div><pre>${esc(professionalSummary())}</pre></section>`
      : noteStep
  ...
  $('[data-v095-score-delta-input]', overlay)?.addEventListener('input', event => {
    wizard.scoreDelta = clamp(event.target.value, -10, 10); $('[data-v095-score-delta]', overlay).textContent = `${wizard.scoreDelta >= 0 ? '+' : ''}${Number(wizard.scoreDelta).toFixed(1)}`; $('[data-v095-subjective-score]', overlay).textContent = subjectiveScore().toFixed(1);
  });
  $('[data-v095-radar-value]', overlay)?.addEventListener('input', event => {
    if (!wizard.selectedRadar) return; wizard.radar[wizard.selectedRadar.group][wizard.selectedRadar.index] = Number(event.target.value); $('[data-v095-radar-output]', overlay).textContent = Number(event.target.value).toFixed(1); updateRadarChart(overlay, wizard.selectedRadar.group);
  });
  $$('[data-v095-defect-group]', overlay).forEach(button => button.addEventListener('click', () => {
    const list = wizard.defects[button.dataset.v095DefectGroup]; const tag = button.dataset.v095DefectTag; const index = list.indexOf(tag);
    if (index >= 0) list.splice(index, 1); else list.push(tag); button.classList.toggle('active', index < 0); button.setAttribute('aria-pressed', String(index < 0));
  }));
  bindSelectedTagSorting(overlay, sensoryStep ? step.id : ''); bindRadarInteractions(overlay);
  $('[data-v095-prev]', overlay)?.addEventListener('click', () => { wizard.step = Math.max(0, wizard.step - 1); renderWizard(); });
  $('[data-v095-next]', overlay)?.addEventListener('click', async () => { if (wizard.step < SUMMARY_STEP) { wizard.step += 1; renderWizard(); } else await finishProfessional(); });
}

function professionalSummary() {
  const lines = [];
  for (const step of STEPS) { const tags = wizard.selections[step.id]; if (tags.length) lines.push(`${step.title}：${tags.join('、')}；强度 ${Number(wizard.intensities[step.id]).toFixed(1)}`); }
  ...
  const defects = Object.entries(wizard.defects).flatMap(([key, tags]) => tags.map(tag => `${DEFECT_GROUPS[key].label}-${tag}`)); if (defects.length) lines.push(`缺陷：${defects.join('、')}`);
  return lines.join('\n');
}
...
function subjectiveScore() { return clamp(affectiveMappedScore() + Number(wizard?.scoreDelta || 0), 0, 100); }

async function finishProfessional() {
  if (!wizard || transferBusy) return;
  transferBusy = true;
  const detail = {
    recordId: wizard.editRecordId, beanId: wizard.beanId, brewSessionId: wizard.brewSessionId, planReference: wizard.planReference, profileId: wizard.profileId, source: wizard.source,
    autoScore: affectiveMappedScore(), score: subjectiveScore(), scoreDelta: Number(wizard.scoreDelta || 0), summary: professionalSummary().split('\n').filter(Boolean), naturalNote: wizard.naturalNote.trim(),
    professionalData: { selections: structuredClone(wizard.selections), intensities: structuredClone(wizard.intensities), radar: structuredClone(wizard.radar), defects: structuredClone(wizard.defects), scoring: { autoScore: affectiveMappedScore(), subjectiveScore: subjectiveScore(), scoreDelta: Number(wizard.scoreDelta || 0) } }
  };
  closeWizard(); document.dispatchEvent(new CustomEvent('luckybean:professional-sensory-complete', { detail })); transferBusy = false;
}
...
document.addEventListener('luckybean:edit-professional-sensory', event => { const record = event.detail?.record; if (!record?.id || record.evaluationMode !== 'professional') return; startMode('professional', record); });
