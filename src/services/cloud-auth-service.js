...
function writeSession(value) { if (value?.access_token || value?.refresh_token) localStorage.setItem(SESSION_KEY, JSON.stringify(value)); else localStorage.removeItem(SESSION_KEY); }
function rememberUntil() { return Number(localStorage.getItem(REMEMBER_UNTIL_KEY) || 0); }
function markServerActivity() { const now = Date.now(); localStorage.setItem(LAST_SERVER_ACTIVITY_KEY, String(now)); localStorage.setItem(REMEMBER_UNTIL_KEY, String(now + REMEMBER_MS)); }
function rememberPendingRegistration(email) { localStorage.setItem(PENDING_REGISTRATION_KEY, JSON.stringify({ email:String(email || '').toLowerCase(), createdAt:Date.now() })); }
function readPendingRegistration() {
  try {
    const value = JSON.parse(localStorage.getItem(PENDING_REGISTRATION_KEY) || 'null');
    if (!value?.email || Date.now() - Number(value.createdAt || 0) > PENDING_REGISTRATION_MS) { localStorage.removeItem(PENDING_REGISTRATION_KEY); return null; }
    return value;
  } catch { localStorage.removeItem(PENDING_REGISTRATION_KEY); return null; }
}
function clearPendingRegistration() { localStorage.removeItem(PENDING_REGISTRATION_KEY); }
...
function dispatchLoginSuccess(payload, authAction) {
  document.dispatchEvent(new CustomEvent('luckybean:cloud-login-success', { detail:{ user:payload?.user || null, authAction } }));
}
...
async function signOut() {
  ...
  writeSession(null); localStorage.removeItem(REMEMBER_UNTIL_KEY); emit('signed-out');
}
