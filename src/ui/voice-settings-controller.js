...
function config() {
  return parse(localStorage.getItem(VOICE_KEY), { mode: 'auto', voiceURI: '', rate: 1.05 });
}

function save(next) {
  localStorage.setItem(VOICE_KEY, JSON.stringify({
    mode: ['female', 'male'].includes(next.mode) ? next.mode : 'auto',
    voiceURI: String(next.voiceURI || ''),
    rate: Math.min(1.3, Math.max(.75, Number(next.rate) || 1.05))
  }));
}
...
function render() {
  ...
  categories.querySelector('[data-voice-settings]')?.remove();
  const current = config();
  const section = document.createElement('details');
  section.className = 'settings-category';
  section.dataset.voiceSettings = '1';
  const available = chineseVoices();
  section.innerHTML = `<summary><span>语音</span><small>声音、语速与试听</small></summary>
    <div class="settings-category-body">
      <div class="grid-2">
        <label class="field"><span>声音倾向</span><select class="control" data-voice-mode><option value="auto">自动</option><option value="female">偏女声</option><option value="male">偏男声</option></select></label>
        <label class="field"><span>当前设备声音</span><select class="control" data-voice-select><option value="">按倾向自动选择</option>${available.map(voice => { const key = voice.voiceURI || `${voice.name}|${voice.lang}`; return `<option value="${key.replaceAll('&','&amp;').replaceAll('"','&quot;')}">${voice.name} · ${voice.lang || ''}${voice.localService ? ' · 本地' : ''}</option>`; }).join('')}</select></label>
      </div>
      <label class="field"><span>语速 <output data-voice-rate-output>${Number(current.rate).toFixed(2)}</output></span><input class="control" type="range" min="0.75" max="1.30" step="0.05" value="${Number(current.rate)}" data-voice-rate></label>
  ...
  $('[data-voice-save]', section).addEventListener('click', () => {
    save({ mode: $('[data-voice-mode]', section).value, voiceURI: select.value, rate: Number(rate.value) });
    section.open = false;
  });
}
...
function bind() {
  ...
  if (root && root.dataset.voiceObserver !== '1') {
    root.dataset.voiceObserver = '1';
    new MutationObserver(queueRender).observe(root, { childList: true });
  }
  queueRender();
}
