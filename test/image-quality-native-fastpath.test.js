...
test('native Android OCR bypasses WebView decode, pixel scan and JPEG re-encode', async () => {
  ...
  try {
    ...
    Object.defineProperty(globalThis, 'document', {
      configurable: true,
      writable: true,
      value: { createElement() { throw new Error('native fast path must not allocate canvas'); } }
    });
    ...
    assert.equal(prepared.processedWidth, 0);
    assert.equal(prepared.processedHeight, 0);
