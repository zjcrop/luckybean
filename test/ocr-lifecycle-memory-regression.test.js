...
test('OCR provider module is registered before package capture without eager model warmup', async () => {
  ...
  assert.match(preinteraction, /'recognition-paddle-ocr'/);
  assert.match(preinteraction, /'package-capture'/);
  assert.ok(preinteraction.indexOf("'recognition-paddle-ocr'") < preinteraction.indexOf("'package-capture'"));
