...
test('refinement runtime files are loaded and cached', async () => {
  ...
  for (const marker of [
    'styles-v095-refine.css?v=095c',
    'src/v095-layout-gear.js?v=095c',
    'src/v095-sensory-pro.js?v=095c'
  ]) assert.ok(html.includes(marker), marker);
  assert.match(sw, /luckybean-v0\.9\.5-refine3/);
  for (const marker of ['./styles-v095-refine.css', './src/v095-layout-gear.js', './src/v095-sensory-pro.js']) {
...
test('light mode covers seals, groups, leaderboard and collapse row', async () => {
  ...
  for (const marker of [
    'html[data-theme="light"] .page-seal',
    'color: #fff !important',
    '.group-card',
    '.active-group-panel',
    '.group-collapse',
    '.preference-board-line',
    '.recommendation-board button',
    '.overlay[data-overlay="recommendation-board"] .dialog'
...
test('remaining recommendation, manage menu and structured grinder range are implemented', async () => {
  ...
  for (const marker of [
    "value.replaceAll('拾余', '余量')",
    "dot.style.background = '#8b8b87'",
    "$$('[data-manage-action=\"history\"]')",
    '手冲常用刻度范围',
    'rangeStart',
    'rangeEnd',
    'normalizeDripperSection',
    'serializeGrinders'
  ]) assert.ok(layout.includes(marker), marker);
});
...
test('three sensory modes have distinct non-duplicated workflows', async () => {
  ...
  for (const marker of [
    ...
    'skipNativeToScore',
    'finishProfessional',
    'injectProfessionalNote'
  ]) assert.ok(sensory.includes(marker), marker);
...
test('professional sensory uses CATA tags, ordering, conditional radar slider and final affective score', async () => {
  ...
  for (const marker of [
    ...
    "chip.addEventListener('pointermove'",
    'data-radar-axis',
    'data-radar-slider',
    "if (!wizard.selectedRadar || wizard.selectedRadar.key !== key)",
    'data-affective-value',
    '确认评分，进入札记',
    '不是 SCA 官方总分公式'
  ]) assert.ok(sensory.includes(marker), marker);
});
