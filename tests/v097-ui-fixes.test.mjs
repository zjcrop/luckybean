...
test('explicit segment selector constrains the optimizer while legacy count alone does not', async () => {
  ...
  const plan = await computeFallbackPlan({
    ...baseInput,
    brew: { ...baseInput.brew, profileId: 'recommended', segmentMode: '3', segments: 3 }
  });
...
test('v097 runtime restores old trajectory semantics and requested interactions', async () => {
  ...
  for (const marker of [
    'v097-flavor-coverage',
    'trajectory-series.floral',
    'trajectory-window.positive',
...
test('brew history abbreviation follows five-character rule', async () => {
  ...
  assert.match(runtime, /chars\.length > maximum/);
  assert.match(runtime, /chars\.slice\(0,maximum\)|chars\.slice\(0, maximum\)/);
