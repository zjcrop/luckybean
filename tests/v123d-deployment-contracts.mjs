...
for (const runtimeModule of ['release-1.24b-ui-policy.js','release-1.24b-brew-mode-controller.js','release-1.24b-freshness-detail.js','recognition-batch-progress-controller.js','sensory-tag-sort-controller.js']) assert.ok(runtimeFeatures.includes(runtimeModule), `missing runtime 1.24B module ${runtimeModule}`);
...
assert.doesNotMatch(releaseGroup, /data-v099t-group-back|back\.click/);
...
assert.match(deployWorkflow, /longpress-pointer-capture/);
...
assert.match(buildWorkflow, /longpress-pointer-capture/);
...
assert.match(androidWorkflow, /longpress-pointer-capture/);
assert.match(androidWorkflow, /sensory-tag-sort-controller\.js/);
assert.match(androidBuild, /LUCKYBEAN_KEYSTORE_FILE/);

console.log(`LuckyBean 1.24B ${releaseRevision} direct Pages, signed Android, canonical group state, sensory sort, serial OCR, lifecycle, frozen freshness and BrewProfiles contracts passed`);
