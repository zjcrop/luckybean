...
test.beforeEach(async ({ page }) => {
  ...
  await page.goto(`${BASE_URL}/?v123e-canonical-interaction=1`, { waitUntil:'domcontentloaded' });
...
test('bean flavor picker is regrouped to five requested categories and uses dialog as the only scroller', async ({ page }) => {
  await page.evaluate(() => {
    document.querySelector('#overlayRoot').innerHTML = `<div class="overlay full" data-overlay="flavors"><div class="dialog" style="height:320px"><div class="flavor-groups"><section class="flavor-group"><h3>旧分类</h3><div class="flavor-grid">
    ...
    document.querySelector('[data-overlay="flavors"] .dialog').scrollTop = 100;
  });
  ...
  await expect(groups.nth(0)).toContainText('茉莉');
  await expect(groups.nth(4)).toContainText('巧克力');
  expect(await page.locator('[data-overlay="flavors"] .dialog').evaluate(node => node.scrollTop)).toBe(0);
  await expect(page.locator('[data-overlay="flavors"]')).toHaveCSS('overflow-y', 'hidden');
  await expect(page.locator('[data-overlay="flavors"] .dialog')).toHaveCSS('overflow-y', 'auto');
  await expect(page.locator('[data-overlay="flavors"] .flavor-button').first()).toHaveCSS('border-radius', '7px');
});

test('professional sensory flavor tags use the canonical rounded-rectangle component', async ({ page }) => {
  await page.evaluate(() => {
    const node = document.createElement('div');
    node.className = 'v095-tag-grid';
    node.innerHTML = '<button class="cupping-flavor-tag" data-v095-tag="花香">花香</button><button class="cupping-flavor-tag selected" data-v095-tag="果香">果香</button>';
    document.body.append(node);
  });
  await expect(page.locator('[data-v095-tag="花香"]')).toHaveCSS('border-radius', '7px');
...
test('automatic profile recommendation is made visible when BrewProfiles returns a match', async ({ page }) => {
  ...
  await expect(panel).toContainText('自动');
