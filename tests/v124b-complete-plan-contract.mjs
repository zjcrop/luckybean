...
assert.doesNotMatch(groupNav,/data-v099t-group-back|back\.click/);
...
for(const file of ['release-1.24b-freshness-detail.js','recognition-batch-progress-controller.js','recognition-field-resolver-1.24b.js','release-1.24b-transit-controller.js','release-1.24b-group-navigation.js','release-1.24b-about-controller.js','release-1.24b-brew-mode-controller.js','release-1.24b-ui-policy.js','sensory-tag-sort-controller.js','local-brew-recipes-1.24b.js','grind-psd-reference-service.js','order-recognition-1.24b.js']) assert.ok(sw.includes(file),`service worker missing ${file}`);
assert.match(deploy,/push:[\s\S]*branches: \[main\]/);
assert.match(deploy,/deploy-pages@v5\.0\.0/);
assert.match(deploy,/canonical-state-api/);
assert.match(deploy,/longpress-pointer-capture/);
...
assert.match(build,/longpress-pointer-capture/);
assert.match(build,/CERT_SHA256\.txt/);
assert.match(build,/release-status/);

console.log('LuckyBean 1.24B main.4 complete modification-plan contract passed');
