...
assert.match(policy, /\.preference-board-strip\s*\{[^}]*display:\s*none\s*!important\s*;/s);
assert.doesNotMatch(policy, /\.preference-board-strip\s*\{[^}]*display:\s*flex\s*;/s);
...
assert.match(policy, /\.bean-consumption-summary \.lb-stock-total\s*\{[^}]*font-size:\s*\.94rem\s*!important/s);
assert.match(policy, /#beanGroups \.bean-group-dismiss-surface\s*\{[^}]*min-height:\s*clamp\(100px,\s*16vh,\s*160px\)\s*!important/s);
...
assert.match(app, /openBeanGroup\(groupKey\(bean, state\.settings\.groupMethod \|\| 'country'\), \{ animation: state\.groupAnimationMode \}\)/);
assert.doesNotMatch(app, /recommendationExpandedAll|recommendation-all-groups/);
assert.match(app, /\['price', '价冠', '#000000', false\]/);
assert.match(app, /\['remaining', '拾余', '#808080', false\]/);
