...
async function openApp(page, suffix) {
  ...
  await page.addInitScript(()=>localStorage.setItem('luckybean.onboarding.v2',JSON.stringify({stage:'existing-user',updatedAt:new Date().toISOString(),reason:'v124p-preflight-test'})));
...
test('native OCR auto-handoff still requires canonical preflight confirmation before populating the bean form', async ({ page }) => {
  ...
  // Camera input must immediately start OCR; no manual recognition control exists.
  ...
  await expect(page.locator('[data-overlay="recognition-preflight"]')).toBeVisible({ timeout: 15000 });
...
test('pending text entity enters preflight before explicit bean-form confirmation', async ({ page }) => {
  await openApp(page, 'v124p-preflight-pending=1');
  await page.evaluate(() => globalThis.LuckyBeanRecognitionFlow.openText('COUNTRY ATLANTIS'));
  await page.locator('#parseTextBtn').click();

  const preflight = page.locator('[data-overlay="recognition-preflight"]');
  await expect(preflight).toBeVisible({ timeout: 10000 });
  await expect(preflight).toContainText('ATLANTIS');
  await confirmRecognitionPreflight(page);
  ...
  const reviewRow = form.getByRole('button', { name:/编辑并确认国家/ });
  await expect(pending).toBeVisible();
  await expect(reviewRow).toBeVisible();
  await reviewRow.click();
  const confirmField = form.locator('[data-confirm-recognition-field="countryCode"]');
